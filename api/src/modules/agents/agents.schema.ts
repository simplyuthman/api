...
const allowedAgentSortFields = ["name", "email", "createdAt"] as const;
...
export const listAgentsQuerySchema = z.object({
  ...
  sort: z
    ...
    .optional()
    .refine(
      (val) => val === undefined || (allowedAgentSortFields as readonly string[]).includes(val),
      {
  ...
  agencyId: z.string().uuid({ message: "agencyId is not a valid identifier" }).optional(),
});
